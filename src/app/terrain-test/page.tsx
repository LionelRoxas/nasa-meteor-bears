...
export default function TerrainTestPage() {
  ...
  const [simulationPhase, setSimulationPhase] = useState<'approach' | 'impact' | 'aftermath'>('approach');

  const { getEnhancedPrediction, loading: predictionLoading } = useEnhancedPredictions();
  const [enhancedData, setEnhancedData] = useState<EnhancedPrediction | null>(null);
  ...
  const handleGetEnhancedData = async () => {
    ...
    if (prediction?.impact_location) {
      ...
      console.log("Updated to real impact coordinates:", prediction.impact_location.latitude, prediction.impact_location.longitude);
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">
            Temporary development space for enhanced 2D terrain visualization with 3D→2D transition
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-gray-800 rounded-lg p-6 space-y-6">
              {/* Asteroid Selection */}
              <div>
                <h3 className="text-lg font-semibold mb-3 text-blue-400">Asteroid Selection</h3>
              ...
              <div>
                <h3 className="text-lg font-semibold mb-3 text-purple-400">AI Analysis</h3>
                <button
                ...
                >
                  {predictionLoading ? 'Analyzing...' : 'Get Enhanced Prediction'}
                </button>

                {enhancedData && (
                  <div className="mt-3 p-3 bg-gray-700 rounded">
                    <div className="text-sm space-y-1">
                      <div>Risk Score: <span className="text-red-400">{enhancedData.risk_score}%</span></div>
                      <div>Threat: <span className="text-orange-400">{enhancedData.threat_category}</span></div>
                      <div>Correlations: <span className="text-blue-400">{enhancedData.correlation_context.top_similar_earthquakes}</span></div>
              ...
              {/* Simulation Controls */}
              <div>
                <h3 className="text-lg font-semibold mb-3 text-green-400">Simulation</h3>
                <div className="space-y-3">
                  <div className="text-sm">
                    <div>Phase: <span className="capitalize text-green-400">{simulationPhase}</span></div>
                    <div>Location: {impactLocation.lat.toFixed(2)}°, {impactLocation.lng.toFixed(2)}°</div>
                  </div>
                  ...
                  {isSimulating && (
                    <div className="text-center">
                      <div className="text-sm text-gray-400 mb-2">Simulation Phase</div>
                      <div className="flex justify-center space-x-2">
                        <div className={`w-3 h-3 rounded-full ${simulationPhase === 'approach' ? 'bg-yellow-400 animate-pulse' : 'bg-gray-600'}`}></div>
                        <div className={`w-3 h-3 rounded-full ${simulationPhase === 'impact' ? 'bg-red-400 animate-pulse' : 'bg-gray-600'}`}></div>
                        <div className={`w-3 h-3 rounded-full ${simulationPhase === 'aftermath' ? 'bg-blue-400 animate-pulse' : 'bg-gray-600'}`}></div>
                      </div>
              ...
              <div>
                <h3 className="text-lg font-semibold mb-3 text-orange-400">Impact Data</h3>
                <div className="bg-gray-700 rounded p-3 space-y-2 text-sm">
                  <div>Energy: <span className="text-orange-400">{selectedAsteroid.energy} MT TNT</span></div>
                  <div>Crater: <span className="text-red-400">{selectedAsteroid.craterRadius} km</span></div>
                  <div>Diameter: <span className="text-blue-400">{(selectedAsteroid.diameter/1000).toFixed(1)} km</span></div>
                  <div>Velocity: <span className="text-green-400">{selectedAsteroid.velocity} km/s</span></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-gray-800 rounded-lg p-6">
              <TerrainVisualizer
                ...
                onImpactLocationChange={(lat, lng) => setImpactLocation({ lat, lng })}
        ...
        <div className="mt-8 p-4 bg-yellow-900/30 border border-yellow-700 rounded-lg">
          <div className="flex items-start gap-2">
            ...
            <div>
              <div className="text-yellow-400 font-medium">Temporary Development Space</div>
              <div className="text-yellow-300 text-sm mt-1">
                This is a standalone testing environment for the 2D terrain visualizer.
                Once we&apos;re happy with the implementation, we&apos;ll integrate it into the main application.
