...
interface CorrelatedEarthquake {
  magnitude: number;
  location: string;
  time: string;
  depth_km: number;
  ...
  try {
    ...
    if (!asteroidId || !context || !asteroidData) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: asteroidId, context, and asteroidData",
        },
        { status: 400 }
      );
    }
    ...
    console.log(
      `Generating enhanced prediction for asteroid ${asteroidId} using LLM`
    );
    ...
    if (!groqApiKey) {
      ...
      return NextResponse.json(
        generateFallbackPrediction(asteroidData, correlatedEarthquakes)
      );
    }
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert astrophysicist and seismic analyst specializing in asteroid impact assessment. Analyze the provided data and return a JSON response with detailed risk assessment.",
            },
            {
              role: "user",
              content: prompt,
    ...
    const enhancedPrediction = processLLMResponse(
      llmAnalysis,
      asteroidData,
    ...
    console.log(
      `Enhanced prediction generated successfully for asteroid ${asteroidId}`
    );
...
function buildLLMPrompt(
  ...
  const earthquakeSummary = correlatedEarthquakes
    .map((eq) => `${eq.magnitude}M at ${eq.location} (${eq.time})`)
    .join(", ");
...
Asteroid Data:
- Diameter: ${asteroidData.diameter_km} km
- Kinetic Energy: ${asteroidData.kinetic_energy_mt} MT TNT
- Threat Level: ${asteroidData.threat_level}
- Orbit Class: ${asteroidData.orbit_class}
- Is Hazardous: ${asteroidData.is_hazardous}
- Is Sentry Object: ${asteroidData.is_sentry_object}

Correlated Earthquakes (${correlatedEarthquakes.length} events):
${earthquakeSummary}
...
function processLLMResponse(
  ...
  const correlationScores = correlatedEarthquakes.map((eq) =>
    parseFloat(eq.correlationScore)
  );
  const avgCorrelationScore =
    correlationScores.length > 0
      ? correlationScores.reduce((a, b) => a + b, 0) / correlationScores.length
      : 0;

  // Ensure hazard probability is reasonable based on NASA data
  let hazardProbability = Math.max(
    0,
    Math.min(1, llmAnalysis.hazard_probability || 0.1)
  );
  ...
  return {
    ...
    recommendation:
      llmAnalysis.llm_analysis?.recommendation_detailed ||
    ...
    llm_analysis: {
      historical_comparison:
        llmAnalysis.llm_analysis?.historical_comparison ||
        "Limited historical correlation data available",
      risk_factors: Array.isArray(llmAnalysis.llm_analysis?.risk_factors)
        ? llmAnalysis.llm_analysis.risk_factors
        : ["Insufficient data"],
      confidence_factors: Array.isArray(
        llmAnalysis.llm_analysis?.confidence_factors
      )
        ? llmAnalysis.llm_analysis.confidence_factors
        : ["NASA classification"],
      similar_events:
        llmAnalysis.llm_analysis?.similar_events ||
        "No directly similar events identified",
      impact_assessment:
        llmAnalysis.llm_analysis?.impact_assessment ||
        "Impact assessment requires additional modeling",
    ...
    correlation_context: {
      ...
      energy_comparison_summary:
        llmAnalysis.correlation_context?.energy_comparison_summary ||
        "Energy comparison analysis pending",
      hazard_pattern_analysis:
        llmAnalysis.correlation_context?.hazard_pattern_analysis ||
        "Pattern analysis requires additional data",
    },
  };
}
...
function generateFallbackPrediction(
  ...
  const correlationScores =
    correlatedEarthquakes?.map((eq) => parseFloat(eq.correlationScore)) || [];
  const avgCorrelationScore =
    correlationScores.length > 0
      ? correlationScores.reduce((a, b) => a + b, 0) / correlationScores.length
      : 0;
  ...
  return {
    ...
    llm_analysis: {
      historical_comparison:
        "LLM analysis not available - using correlation scores for basic comparison",
      risk_factors: asteroidData?.is_hazardous
        ? ["NASA hazardous classification", "Large size", "Close approach"]
        : ["Standard orbital parameters"],
      confidence_factors: [
        "NASA official classification",
        "Orbital data quality",
      ],
      similar_events: `${
        correlatedEarthquakes?.length || 0
      } correlated seismic events identified`,
