...
export default function TerrainVisualizer({
  ...
  const generateTerrain = useCallback(() => {
    ...
    for (let y = 0; y < gridHeight; y++) {
      ...
      for (let x = 0; x < gridWidth; x++) {
        const elevation = generateRealisticElevation(
          x,
          y,
          gridWidth,
          gridHeight
  ...
  const generateRealisticElevation = (
    x: number,
    y: number,
    gridWidth: number,
    gridHeight: number
    ...
    for (let i = 0; i < 6; i++) {
      elevation +=
        amplitude *
        (Math.sin(nx * frequency * Math.PI * 2) *
          Math.cos(ny * frequency * Math.PI * 2));
      amplitude *= 0.5;
      frequency *= 2;
    }
  ...
  const determineBiome = (
    ...
    if (latitude < 0.3 && elevation < 0.1 && Math.random() > 0.7)
      return "desert";
    return "land";
  };

  const determinePopulation = (
  ...
  const getBiomeColor = (
    ...
    return `rgb(${Math.floor(base.r * elevationFactor)}, ${Math.floor(
      base.g * elevationFactor
    )}, ${Math.floor(base.b * elevationFactor)})`;
  };
  ...
  const renderTerrain = useCallback(() => {
    ...
    terrain.forEach((row) => {
      row.forEach((point) => {
        ...
        if (point.biome === "ocean") {
          ...
          ctx.globalAlpha =
            0.8 + Math.sin(animationTime + point.x * 0.05) * 0.1;
    ...
    if (asteroidData) {
      ...
      ctx.fillStyle = `rgba(255, 0, 0, ${
        0.3 + Math.sin(animationTime * 4) * 0.1
      })`;
      ctx.fill();
      ...
      ctx.strokeStyle = `rgba(255, 100, 0, ${
        0.5 + Math.sin(animationTime * 3) * 0.2
      })`;
      ctx.lineWidth = 2;
      ctx.stroke();
      ...
      for (let i = 1; i <= 3; i++) {
        const seismicRadius =
          blastRadius + ((animationTime * 50 + i * 30) % 200);
        ctx.beginPath();
        ctx.arc(impactPoint.x, impactPoint.y, seismicRadius, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(255, 255, 0, ${Math.max(
          0,
          0.4 - seismicRadius / 500
        )})`;
        ctx.lineWidth = 1;
        ctx.stroke();
      }
