...
export default function TerrainVisualizer({
  ...
  const [debrisParticles, setDebrisParticles] = useState<Array<{x: number, y: number, vx: number, vy: number, life: number}>>([]);
  const [craterDepth, setCraterDepth] = useState(0);
  const [seismicWaves, setSeismicWaves] = useState<Array<{radius: number, intensity: number, phase: number}>>([]);
  ...
  const simulateRealisticTerrain = useCallback(async (centerLat: number, centerLng: number) => {
  ...
  const generateRegionSpecificElevation = (lat: number, lng: number, terrainData: {
  ...
  const determineRealisticBiome = (elevation: number, lat: number, lng: number, terrainData: {
    terrainType: string;
  }): TerrainPoint["biome"] => {
    if (terrainData.terrainType === 'ocean' || elevation < -0.1) return "ocean";
    if (terrainData.terrainType === 'mountain' || elevation > 0.6) return "mountain";
    if (terrainData.terrainType === 'ice' || Math.abs(lat) > 65) return "ice";
    if (terrainData.terrainType === 'desert') return "desert";
    return "land";
  };

  const determineRealisticPopulation = (biome: TerrainPoint["biome"], elevation: number, lat: number, lng: number, terrainData: {
  ...
  const generateTerrain = useCallback(async () => {
    ...
    if (enhancedPrediction?.impact_location) {
      ...
      console.log("Using real trajectory coordinates from prediction:", impactLat, impactLng);
    } else {
      console.log("Using fallback coordinates:", impactLat, impactLng);
    }

    console.log("Generating realistic terrain for impact at:", impactLat, impactLng);
    ...
    const terrainSpanLng = viewRadius / (111 * Math.cos(impactLat * Math.PI / 180));

    console.log(`Terrain view: ${viewRadius.toFixed(1)}km radius for ${realCraterKm.toFixed(2)}km crater (${realMegatons.toFixed(2)} MT)`);

    console.log(`Showing ${viewRadius}km radius around impact (${terrainSpanLat.toFixed(2)}° x ${terrainSpanLng.toFixed(2)}°)`);

    for (let y = 0; y < gridHeight; y++) {
      const row: TerrainPoint[] = [];
      for (let x = 0; x < gridWidth; x++) {
        const lat = impactLat + (y - gridHeight / 2) * (terrainSpanLat / gridHeight);
        const lng = impactLng + (x - gridWidth / 2) * (terrainSpanLng / gridWidth);

        const elevation = generateRegionSpecificElevation(lat, lng, terrainData);
        const biome = determineRealisticBiome(elevation, lat, lng, terrainData);
        const populated = determineRealisticPopulation(biome, elevation, lat, lng, terrainData);
  ...
  }, [width, height, asteroidData, enhancedPrediction, realCraterDiameter, realAffectedRadius, realMegatons, getRealisticTerrain]);

  // Update real impact physics data when enhanced prediction changes
  useEffect(() => {
    if (enhancedPrediction?.impact_physics) {
      console.log("Updating crater with real physics data:", enhancedPrediction.impact_physics);
      setRealCraterDiameter(enhancedPrediction.impact_physics.craterDiameter ?? 0);
      setRealAffectedRadius(enhancedPrediction.impact_physics.affectedRadius ?? 0);
      setRealEnergy(enhancedPrediction.impact_physics.energy ?? 0);
      setRealMegatons(enhancedPrediction.impact_physics.megatonsEquivalent ?? 0);
    } else {
  ...
  const getBiomeColor = (
    ...
    switch (biome) {
      case "ocean":
        ...
        r = Math.floor(baseColors.ocean.deep.r * oceanMix + baseColors.ocean.shallow.r * (1 - oceanMix));
        g = Math.floor(baseColors.ocean.deep.g * oceanMix + baseColors.ocean.shallow.g * (1 - oceanMix));
        b = Math.floor(baseColors.ocean.deep.b * oceanMix + baseColors.ocean.shallow.b * (1 - oceanMix));
        break;
      ...
      case "land":
        ...
        if (elevation < 0.2) {
          ...
          r = Math.floor(baseColors.land.low.r * (1 - mix) + baseColors.land.mid.r * mix);
          g = Math.floor(baseColors.land.low.g * (1 - mix) + baseColors.land.mid.g * mix);
          b = Math.floor(baseColors.land.low.b * (1 - mix) + baseColors.land.mid.b * mix);
        } else {
          const mix = Math.min(1, (elevation - 0.2) / 0.3);
          r = Math.floor(baseColors.land.mid.r * (1 - mix) + baseColors.land.high.r * mix);
          g = Math.floor(baseColors.land.mid.g * (1 - mix) + baseColors.land.high.g * mix);
          b = Math.floor(baseColors.land.mid.b * (1 - mix) + baseColors.land.high.b * mix);
        }
        break;
      ...
      case "mountain":
        ...
        r = Math.floor(baseColors.mountain.base.r * (1 - mountainMix) + baseColors.mountain.peak.r * mountainMix);
        g = Math.floor(baseColors.mountain.base.g * (1 - mountainMix) + baseColors.mountain.peak.g * mountainMix);
        b = Math.floor(baseColors.mountain.base.b * (1 - mountainMix) + baseColors.mountain.peak.b * mountainMix);
        break;

      case "ice":
        // Ice varies slightly with elevation
        const iceMix = Math.min(1, elevation / 0.8);
        r = Math.floor(baseColors.ice.base.r * (1 - iceMix) + baseColors.ice.bright.r * iceMix);
        g = Math.floor(baseColors.ice.base.g * (1 - iceMix) + baseColors.ice.bright.g * iceMix);
        b = Math.floor(baseColors.ice.base.b * (1 - iceMix) + baseColors.ice.bright.b * iceMix);
        break;

      case "desert":
        // Desert varies between sand and rock
        const desertMix = Math.min(1, elevation / 0.4);
        r = Math.floor(baseColors.desert.sand.r * (1 - desertMix) + baseColors.desert.rock.r * desertMix);
        g = Math.floor(baseColors.desert.sand.g * (1 - desertMix) + baseColors.desert.rock.g * desertMix);
        b = Math.floor(baseColors.desert.sand.b * (1 - desertMix) + baseColors.desert.rock.b * desertMix);
        break;
  ...
  const updatePhysics = useCallback(() => {
    ...
    setDebrisParticles(prev => prev.map(particle => ({
      ...particle,
    ...
    setSeismicWaves(prev => prev.map(wave => ({
      ...wave,
  ...
  const renderTerrain = useCallback(() => {
    ...
    if (terrain.length === 0) {
      console.log("No terrain data available, terrain array length:", terrain.length);
      return;
    }
    ...
    const bgColor = simulationPhase === 'impact'
      ? `rgba(${Math.floor(20 + impactIntensity * 50)}, ${Math.floor(20 + impactIntensity * 30)}, ${Math.floor(30 + impactIntensity * 20)}, 1)`
      : "#001122";
    ...
    if (cameraAngle > 0) {
      ...
      ctx.transform(1, 0, 0, Math.cos(cameraAngle), 0, height * (1 - Math.cos(cameraAngle)) / 2);
    }
    ...
    terrain.forEach((row) => {
      row.forEach((point) => {
        const distanceFromImpact = Math.sqrt(
          Math.pow(point.x - impactPoint.x, 2) + Math.pow(point.y - impactPoint.y, 2)
        );
        ...
        if (showImpact && simulationPhase === 'impact') {
          ...
          const realAffectedKm = realAffectedRadius || (asteroidData?.craterRadius || 50) * 8;
          const impactEffectPixels = realAffectedKm * 3; // Convert km to pixels (3 pixels per km)
          const impactEffect = Math.max(0, 1 - distanceFromImpact / impactEffectPixels);
        ...
        if (simulationPhase === 'aftermath') {
          const realAftermathKm = (realAffectedRadius || (asteroidData?.craterRadius || 50) * 6) * 0.8; // Slightly smaller than impact
          const aftermathPixels = realAftermathKm * 3; // Convert km to pixels
          const aftermathEffect = Math.max(0, 1 - distanceFromImpact / aftermathPixels);
        ...
        if (point.biome === "ocean") {
          ...
          const waveOffset = Math.sin(animationTime * 1.5 + point.x * 0.08 + point.y * 0.06) * 1.5;
          const waveIntensity = 0.85 + Math.sin(animationTime * 0.8 + point.x * 0.04) * 0.15;

          ctx.globalAlpha = waveIntensity;
          ctx.fillRect(projectedPoint.x, projectedPoint.y + waveOffset, pixelSize, pixelSize);
          ...
          ctx.fillRect(projectedPoint.x, projectedPoint.y, pixelSize, pixelSize);

          // Add highlights on peaks
          if (point.elevation > 0.8) {
            ctx.fillStyle = `rgba(255, 255, 255, ${(point.elevation - 0.8) * 2})`;
            ctx.fillRect(projectedPoint.x + 1, projectedPoint.y, 2, 2);
          }

          // Add shadows for depth
          ctx.fillStyle = `rgba(0, 0, 0, ${Math.max(0, 0.2 - point.elevation * 0.1)})`;
          ctx.fillRect(projectedPoint.x, projectedPoint.y + 2, pixelSize, 2);

        } else if (point.biome === "desert") {
          // Desert with dune patterns
          const dunePattern = Math.sin(point.x * 0.3) * Math.cos(point.y * 0.2) * 0.1;
          ctx.fillRect(projectedPoint.x, projectedPoint.y + dunePattern, pixelSize, pixelSize);

          // Add sand texture
          if (Math.random() > 0.7) {
            ctx.fillStyle = `rgba(255, 220, 150, 0.3)`;
            ctx.fillRect(projectedPoint.x + Math.random() * 2, projectedPoint.y + Math.random() * 2, 1, 1);
          }
          ...
          if (Math.random() > 0.85) {
            ...
            ctx.fillRect(projectedPoint.x + Math.random() * 3, projectedPoint.y + Math.random() * 3, 1, 1);
          }
        ...
        if (point.populated) {
          if (simulationPhase === 'impact' && distanceFromImpact < (asteroidData?.craterRadius || 0) * 20) {
            // Destroyed settlements (red/orange)
            ctx.fillStyle = "#ff4444";
          } else if (simulationPhase === 'aftermath' && distanceFromImpact < (asteroidData?.craterRadius || 0) * 50) {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    simulationPhase, cameraAngle, impactIntensity, craterDepth, showImpact,
    enhancedPrediction, project3DTo2D, realAffectedRadius
  ]);

  // Render advanced impact effects
  const renderAdvancedImpactEffects = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    console.log(`Rendering crater: ${realCraterKm.toFixed(2)} km = ${craterPixelRadius.toFixed(0)} pixels (Real: ${realMegatons.toFixed(2)} MT)`);

    // Enhanced crater with dramatic depth visualization
    if (showImpact || craterDepth > 0) {
      // Outer crater rim (ejected material)
      ctx.beginPath();
      ctx.arc(impactPoint.x, impactPoint.y, craterPixelRadius * 1.3, 0, Math.PI * 2);
      ...
      for (let r = craterPixelRadius; r > 0; r -= 3) {
        ...
        ctx.fillStyle = `rgba(${Math.floor(20 * (1 - darkness))}, ${Math.floor(10 * (1 - darkness))}, ${Math.floor(5 * (1 - darkness))}, ${0.9 + darkness * 0.1})`;
        ctx.fill();
      }

      // Central crater floor (very dark)
      ctx.beginPath();
      ctx.arc(impactPoint.x, impactPoint.y, craterPixelRadius * 0.3, 0, Math.PI * 2);
      ...
      for (let i = 0; i < 8; i++) {
        ...
        const startX = impactPoint.x + Math.cos(angle) * craterPixelRadius * 0.3;
        const startY = impactPoint.y + Math.sin(angle) * craterPixelRadius * 0.3;
        const endX = impactPoint.x + Math.cos(angle) * craterPixelRadius * 0.9;
        const endY = impactPoint.y + Math.sin(angle) * craterPixelRadius * 0.9;
    ...
    if (shockwaveRadius > 0) {
      ...
      ctx.strokeStyle = `rgba(255, 165, 0, ${Math.max(0, 0.6 - shockwaveRadius / width)})`;
      ctx.lineWidth = 4;
      ctx.stroke();
    }
    ...
    if (simulationPhase === 'impact' && impactIntensity > 0.5) {
      ...
      for (let i = 1; i <= 3; i++) {
        ctx.beginPath();
        ctx.arc(impactPoint.x, impactPoint.y, craterPixelRadius * i * 2, 0, Math.PI * 2);
        const flashIntensity = Math.max(0, (impactIntensity - 0.5) * (1 / i) * 0.8);
      ...
      ctx.beginPath();
      ctx.arc(impactPoint.x, impactPoint.y, craterPixelRadius * 0.8, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(255, 255, 255, ${impactIntensity * 0.9})`;
      ctx.fill();
    }
  }, [asteroidData, impactPoint, craterDepth, shockwaveRadius, seismicWaves, debrisParticles, simulationPhase, impactIntensity, width, realCraterDiameter, realMegatons, showImpact]);

  // Render seismic analysis from enhanced predictions
  const renderSeismicAnalysis = useCallback((ctx: CanvasRenderingContext2D) => {
    if (!enhancedPrediction?.correlation_context) return;

    const earthquakeCount = enhancedPrediction.correlation_context.top_similar_earthquakes ?? 0;
    ...
    for (let i = 0; i < Math.min(earthquakeCount, 8); i++) {
      ...
      ctx.fillStyle = `rgba(100, 200, 255, ${0.6 + Math.sin(animationTime * 2 + i) * 0.3})`;
      ctx.fill();
  ...
  // Render UI overlays
  const renderUIOverlays = useCallback((ctx: CanvasRenderingContext2D) => {
    // Coordinate grid
    ...
    if (cameraAngle > 0) {
      const angleText = `View: 3D (${Math.round(cameraAngle * 180 / Math.PI)}°)`;
    ...
    if (enhancedPrediction) {
      ...
      ctx.fillText(`Risk: ${enhancedPrediction.risk_score}%`, width - 190, 50);
      ctx.fillText(`Threat: ${enhancedPrediction.threat_category}`, width - 190, 70);
      ctx.fillText(`Correlations: ${enhancedPrediction.correlation_context?.top_similar_earthquakes ?? 0}`, width - 190, 90);
    }
